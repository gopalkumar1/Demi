...
def main_function(request):
	...
	if file.filename.endswith(".pdf"):
		...
		if os.stat(def_enum.inputpath + "/form.pdf").st_size > 31457280:
	                x["code"] = 1
	                x["message"] = "not a valid form, form is too large"
	                flush_dir()
	                clc()
		...
		#file.save(def_enum.inputpath + "/form.docx")
		#if os.stat(def_enum.inputpath + "/form.docx").st_size > 1451022:
	#	x["code"] = 1
	#	x["message"] = "docx is not supported"
	#	flush_dir()
	#	clc()
	#	return x
		...
		x["code"] = 1
		x["message"] = "not a valid form, only pdf supported"
		return x
...
##		return x
	...
	#pdf_info = pdfinfo_from_path(name_of_file)
